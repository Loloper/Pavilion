...
def mainmenu(fileentry = ['No files'], fileentryload=[],filetypes=[" ",".sh",".txt",".py",".pdf",".java"],Projname='pav.py', autoauthorsave=[]):
    def editor():
        ...
        def ProjectM():
            #Project viewer
            framep = LabelFrame(ED, text="Project viewer")
            framep.grid(row=0,column=0)
            protxt = Text(framep, width=15, height=47)
            protxt.delete('1.0', END)
            protxt.pack()
            recentconP = subprocess.run('ls', capture_output=True, text=True)
            reccongigP = recentconP.stdout
            protxt.insert(END, reccongigP)
        ...
        def Deletefiles():
            def Dcondel():
                delname = Dentry.get()
                os.remove(delname)
                messagebox.showinfo("Deleted", "Project {0} was deleted".format(delname))
                Droot.destroy()
                ProjectM()
            ...
            def DrootDRY():
                Droot.destroy()
            Droot = Tk()
            Droot.title("Delete files")
            Droot.geometry("500x100")

            Dframe = LabelFrame(Droot)
            Dframe.pack()
            Dlabel = Label(Dframe, text="Delete files", font=("Arial Black", 12), padx=2,pady=5, height=1)
            Dlabel.grid(row=0,column=0)
            Dbackbtn = Button(Dframe, text="Back", width=10, command=DrootDRY)
            Dbackbtn.grid(row=0,column=1)
            Dentry = Entry(Dframe, width=30)
            Dentry.grid(row=1,column=0)
            Dconf = Button(Dframe, text="Delete", width=10, command=Dcondel)
            Dconf.grid(row=1,column=1)
...
#============================================================================
        ...
        def LDconkey(event):
            def recentconD():
                recentcon1D = subprocess.run('ls', capture_output=True, text=True)
                reccongigD = recentcon1D.stdout
                recentfilestxtD.insert(END, reccongigD)
            def LDdestryD():
                LDD.destroy()
            def LDconD():
                lname1D = LName_of_fileentD.get()
                #checks if the program is the source file
                if lname1D == Projname:
                    messagebox.showwarning("Be careful!","You are opening Pavilions source code file, feel free to edit anything you want. Just be careful not to break anything. Happy editing :D")
                ...
                editortxt.delete('1.0', END)
                fileentry.clear()
                fileentry.append(lname1D)
                LDD.destroy()
                #open the chosen file
                ldpD = open(lname1D,"r")
                #paste contents of file on to editor window
                editortxt.insert(END, ldpD.read())
                #close and reopen the file a editable.
                ldpD.close()


            LDD = Tk()
            LDD.title("Load Project")
            LDD.geometry("500x200")

            LDframeD = LabelFrame(LDD)
            LDframeD.pack()
            LTitlenwD = Label(LDframeD, text="Load Project", font=("Arial Black", 12), padx=2,pady=5, height=1)
            LTitlenwD.grid(row=0,column=0)
            LName_of_fileD = Label(LDframeD, text="Name of project:", font=("Arial", 12),padx=10)
            LName_of_fileD.grid(row=1,column=0)
            LName_of_fileentD = Entry(LDframeD, width=30)
            LName_of_fileentD.grid(row=1,column=1)
            LfilelocationbtnD = Button(LDframeD,text="Next",width=10,command=LDconD)
            LfilelocationbtnD.grid(row=1, column=3)
            Lfilelocationbtn1D = Button(LDframeD,text="back",width=10,command=LDdestryD)
            Lfilelocationbtn1D.grid(row=0, column=3)

            #Text box that displays the recent files opened
            LDrecframeD = LabelFrame(LDD, text="Documents in path")
            LDrecframeD.pack()
            recentfilestxtD = Text(LDrecframeD, width=60, height=5)
            recentfilestxtD.pack()
        ...
        ED = Tk()
        filedir = __file__
        ED.title('E D I T O R <{0}>'.format(filedir))
        ...
        frameed.grid(row=0,column=1)
        editortxt = Text(frameed, width=500, height=47) #this is our editor window
        editortxt.delete('1.0', END)
        editortxt.pack()
...
#============================================================================
    ...
    title = Label(text="P A V I L I O N",width=13,height=6, font=("Arial Black", 20))
    title.pack()
